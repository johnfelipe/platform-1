...
module.exports = Backbone.View.extend({
  ...
  initialize: function() {
    ...
    $(Shareabouts).on("visibility", function(evt, id, visible, isBasemap) {
      ...
      if (config.legend && visible) {
        self.loadWmsLegend(config.url, config.id, config.layers);
      } else if (config.legend && !visible) {
        self.removeWmsLegend(config.id);
      }
  ...
  loadWmsLegend(url, id, layers) {
    const layer = Array.isArray(layers) ? layers[0] : layers;
    url =
      url +
      "request=getLegendGraphic&format=image/jpeg" +
      "&legend_options=layout:vertical;columnheight:800;columns:4&layer=" +
      layer +
      "&blah=foo";
    if (!this.wmsLegendControls[id]) {
      this.wmsLegendControls[id] = L.wmsLegend(url, this.map);
    } else {
      this.map.removeControl(this.wmsLegendControls[id]);
      this.wmsLegendControls[id] = null;
      this.wmsLegendControls[id] = L.wmsLegend(url, this.map);
    }
  },

  removeWmsLegend(id) {
    this.wmsLegendControls[id] &&
      this.map.removeControl(this.wmsLegendControls[id]);
    this.wmsLegendControls[id] = null;
  },
